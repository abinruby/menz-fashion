...
exports.addCategory=async (req, res) => {
  ...
  const categoryvalidate=await Category.findOne({categoryname:{$regex:".*"+req.body.category+".*" ,$options:'i'}})
    ...
    categorysave.save((err) => {
      ...
      console.log(err,'error on add category');
      res.redirect('/404')
    }
  }
/* ----------------------------- delete category ----------------------------- */
  ...
  exports.categoryoffer=async (req, res) => {
    try {
      ...
      console.log('finallllllyyyy',categoryId);
      offerServices
