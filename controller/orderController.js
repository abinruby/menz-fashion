...
exports.placeOrderpage = (req, res) => {
  ...
  exports.placeOrder = async (req, res) => {
    ...
    if (req.session.discountprice) {
      ...
      // console.log("total price after coupon"+ typeof(totalPrice));
      ...
      if (req.body["paymentmethod"] === "cod") {
        ...
        const productDetails= await Order.aggregate(
          [
          {
            $match:{_id: ObjectId(savedOrder._id)}
      ...
      for(let i=0;i<productDetails.length;i++){   
        await product.updateOne({_id:productDetails[i].products.item},{$inc:{stock:-productDetails[i].products.quantity}})
      }
...
exports.returnOrder = async (req, res) => {
  ...
  for(let i=0;i<productDetails.length;i++){   
    ...
    console.log(err, "error happened while return order");
    res.redirect('/404')
  }
};
/* --------------------------------- paypal create order --------------------------------- */
